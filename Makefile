...
.PHONY: test
test:
	go test -v -race -cover -coverprofile=coverage.txt -covermode=atomic `go list ./... | grep -v github.com/apache/skywalking-go/reporter/grpc | grep -v artifacts-observability.sixthsense.rakuten.com/sixthsense/sixthsenseGoAgent/test`
